...
def infer_all(raw_data, db):
    ...
    with open(os.path.join(args.dataset, 'pro_sg/item2id.json'), 'r', encoding="utf-8") as f:
        show2id = json.load(f)

    with open(os.path.join(args.dataset, 'pro_sg/user2id.json'), 'r', encoding="utf-8") as f:
    ...
    vae_result = np.loadtxt(pro_dir+'/vae_result.txt', dtype=float)
    dae_result = np.loadtxt(pro_dir+'/dae_result.txt', dtype=float)

    print("recvae_result: ", recvae_result)
    print("vae_result: ", vae_result)
    print("dae_result: ", dae_result)
    if score < recvae_result:
        score = recvae_result
        model_name = 'recvae'
    if score < vae_result:
        score = vae_result
