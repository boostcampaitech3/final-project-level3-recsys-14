...
def check_main_rival_pb(db):
    print("MF-ALS----------------")
    mf_als_output = pd.read_csv('/home/recognizer14/airflow/dags/module_models/rec_rival_pb/mf_als_prog/rec_rival_pb_mf_als_output.csv')
    mf_als_result = prob_evaluate_high(mf_als_output['handle'], mf_als_output['rec_problems'], db)
    print(mf_als_result)
    
    print("BPR----------------")
    bpr_output = pd.read_csv('/home/recognizer14/airflow/dags/module_models/rec_rival_pb/bpr_prog/rec_rival_pb_bpr_output.csv')
    bpr_result = prob_evaluate_high(bpr_output['handle'], bpr_output['rec_problems'], db)
    print(bpr_result)
    
    print("Item-based-CF----------------")
    item_based_output = pd.read_csv('/home/recognizer14/airflow/dags/module_models/rec_rival_pb/item_based_prog/rec_rival_pb_item_based_cf_output.csv')
